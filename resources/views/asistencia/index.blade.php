                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="inline-flex" role="group" aria-label="Button group">
                                                <button
                                                    ...
                                                    <a
                                                        href="{{ route('asistencia.edit', $asistencia->id) }}">Editar</a></button>
                                                <button
                                                    class="h-9 px-3 text-indigo-100 transition-colors duration-150 bg-gray-900 focus:shadow-outline hover:bg-yellow-600">
                                                    <a
                                                        href="{{ route('asistencia.show', $asistencia->id) }}">Mostrar</a></button>

                                                <form method="POST"
                                                    action="{{ route('asistencia.destroy', $asistencia->id) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
